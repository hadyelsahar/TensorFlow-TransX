...
class TransEModel(object):
    ...
    def save(self, sess, fname):
        ...
        saver.save(sess, os.path.join(self.save_path, fname))
...
def main(_):
    ...
    config.save_path = "./trainedmodels/"
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.variable_scope("model", reuse=None, initializer=initializer):
                trainModel = TransEModel(config=config, mode="train")
            ...
            for times in range(config.trainTimes):
                ...
                for batch in range(config.nbatches):
                    ...
                    batch_loss += train_step(ph, pt, pr, nh, nt, nr)
                    # current_step = tf.train.global_step(sess, global_step)

                print "Epoch %s \t Loss %s" % (times, batch_loss)

            trainModel.save(sess, 'model.vec')
